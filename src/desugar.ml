...
let rec depends_on_iterator right_bound iterator_id = 
  ...
  | Index(indexable, idx) -> depends_on_iterator indexable iterator_id || depends_on_iterator idx iterator_id
  | _ -> false
...
let rec desugar_stmt (statement: stmt node) : stmt node list = 
  ...
  | For(iterator, iterable, step_opt, body) -> 
    let desugar_range left right incl =
      ...
      let can_lift_right_bound = not @@ depends_on_iterator right iterator.elt in
      let lifted_right_bound = no_loc @@ Decl("_right", None, right, true) in
      let lifted_reference = no_loc @@ Id "_right" in 
      let useable_right_bound = if can_lift_right_bound then lifted_reference else right in
      ...
      let create_while comp1 comp2 = 
        While(no_loc @@ Bop ((if incl then comp1 else comp2), iterator_node, useable_right_bound), new_body) 
      in
      let forward_while = no_loc @@ create_while Lte Lt in
      let backward_while = no_loc @@ create_while Gte Gt in

      let if_else = no_loc @@ If (no_loc @@ Bop(Lt, left, useable_right_bound), [forward_while], [backward_while]) in
