...
and string_of_stmt_helper: stmt -> string = function
  ...
  | For (iterator, iterable, step, b) -> 
    sprintf "for %s in %s by %s \n\t%s\n" (iterator.elt) (string_of_exp iterable) (string_of_exp @@ value ~default:(no_loc @@ Int 1L) step) (string_of_body b)
