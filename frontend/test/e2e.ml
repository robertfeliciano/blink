...
let test_e2e_manual_ast_success _ =
  ...
  let fn = { elt = { frtyp = RetVal (TInt (TSigned Ti32)); fname = "main"; args = []; body = [ { elt = Ret (Some (no_loc (Int (of_int 1)))); loc = Range.norange } ] }; loc = Range.norange } in
  let prog = Prog ([ fn ], []) in
...
let test_e2e_manual_ast_type_error _ =
  ...
  let vdecl = ("x", Some (TInt (TSigned Ti32)), no_loc (Str "s"), false) in
  let stmt = Decl vdecl in
  let fn = { elt = { frtyp = RetVoid; fname = "f"; args = []; body = [ no_loc stmt ] }; loc = Range.norange } in
  let prog = Prog ([ fn ], []) in
  (match Typing.Type.type_prog prog with
