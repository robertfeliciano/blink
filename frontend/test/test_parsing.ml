...
let assert_ok_parse lexbuf =
  ...
  | Ok _ -> ()
  | Error e -> assert_failure (Printf.sprintf "Parse failed: %s" (Core.Error.to_string_hum e))
...
let test_show_manual_ast _ =
  ...
  let fn = { elt = { frtyp = RetVal (TInt (TSigned Ti32)); fname = "f"; args = [(TInt (TSigned Ti32), "x")]; body = [] }; loc = Ast.Range.norange } in
  let prog = Prog ([fn], []) in
...
let test_show_prog_complex _ =
  let fn1 = { elt = { frtyp = RetVoid; fname = "a"; args = []; body = [] }; loc = Ast.Range.norange } in
  let fn2 = { elt = { frtyp = RetVal (TInt (TSigned Ti32)); fname = "b"; args = []; body = [] }; loc = Ast.Range.norange } in
