...
let test_int _ =
  ...
  let _, ty = Ts.type_exp Tc.empty (mk_node e) None in
...
let test_stmt _ =
  ...
  let _, ts, _ = Ts.type_stmt None Tc.empty RetVoid (mk_node s) false in
  let ty_s =
    match ts with
...
let test_array_and_index _ =
  ...
  let _, aty = Ts.type_exp Tc.empty arr_node None in
  ...
  let _, ity = Ts.type_exp Tc.empty (mk_node idx) None in
...
let test_fn_call_ok _ =
  ...
  let _, ty = Ts.type_exp tc (mk_node call) None in
...
let test_method_call_ok _ =
  ...
  in
  let _, ty = Ts.type_exp tc2 (mk_node call) None in
  assert_equal ty (TInt (TSigned Ti32))
