            ...
            switch (r.tag) {
                ...
                case RefTyTag::RFun: {
                    ...
                    for (size_t i = 0; i < r.args.size(); ++i) {
                        ...
                        if (i + 1 < r.args.size()) s += ", ";
                    }
                    s += ") -> ";
                    if (r.ret.tag == RetTyTag::RetVoid) s += "void";
                    else s += tyToString(*r.ret.val);
                    return s;
                }
            }
            break;
...
std::string fdeclToString(const FDecl& f) {
    ...
    oss << ") -> ";
    switch (f.frtyp.tag) {
        case RetTyTag::RetVoid: oss << "void"; break;
        case RetTyTag::RetVal: oss << tyToString(*f.frtyp.val); break;
    }
    oss << " {\n";
    for (auto &st : f.body) {
